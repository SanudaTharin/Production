...
const insertNutData = (req, res) => {
    ...
    db.query("SELECT cumulative_production FROM nut_machine ORDER BY id DESC LIMIT 1", (err, results) => {
        if (err) {
        ...
        const sql = `INSERT INTO nut_machine (date, time, shift, production, cumulative_production) VALUES (?, ?, ?, ?, ?)`;
...
const getNutData = (req, res) => {
    // Query the database to get all the production data
    db.query("SELECT * FROM nut_machine ORDER BY id DESC", (err, results) => {
        if (err) {
            console.error("Database error:", err);
            return res.status(500).json({ error: "Database query error" });
        }
    ...
    db.query(`
        ...
        FROM (
            SELECT
                Shift_Production,
                CASE
                    WHEN entry_rate != 0 THEN (Shift_Production * 4.29 * 100) / (entry_rate * 60)
                    ELSE 0
                END AS Performance,
                CASE
            ...
            FROM (
                SELECT 
                    CASE 
                        ...
                        THEN (
                            (SELECT IFNULL(SUM(production), 0) FROM nut_machine 
...
module.exports = { insertNutData, getNutData, getnutShift, nutpermonth };
