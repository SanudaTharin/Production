                ...
                SELECT 
                    ...
                    SELECT COUNT(*)
                            ...
                            WHERE (
                                (
                                    TIME(CONVERT_TZ(NOW(), 'UTC', 'Asia/Colombo')) BETWEEN '08:00:00' AND '19:59:59'
                                    AND TIME(time) BETWEEN '08:00:00' AND (SELECT MAX(time) FROM cut_drill_machine WHERE date = DATE(CONVERT_TZ(NOW(), 'UTC', 'Asia/Colombo')))
                                    AND NOT (TIME(time) BETWEEN '12:30:00' AND '13:09:59')
                                )
                                OR
                                (
                                    TIME(CONVERT_TZ(NOW(), 'UTC', 'Asia/Colombo')) BETWEEN '20:00:00' AND '23:59:59'
                                    AND TIME(time) BETWEEN '20:00:00' AND (SELECT MAX(time) FROM cut_drill_machine WHERE date = DATE(CONVERT_TZ(NOW(), 'UTC', 'Asia/Colombo')))
                                )
                                OR
                                (
                                    TIME(CONVERT_TZ(NOW(), 'UTC', 'Asia/Colombo')) BETWEEN '00:00:00' AND '07:59:59'
                                    AND TIME(time) BETWEEN '20:00:00' AND (SELECT MAX(time) FROM cut_drill_machine WHERE date = DATE_SUB(DATE(CONVERT_TZ(NOW(), 'UTC', 'Asia/Colombo')), INTERVAL 1 DAY))
                                )
                            )
                            AND (
                                (
