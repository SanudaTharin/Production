...
const getCuttingTwoShift = (req, res) => {
    ...
    (main_query.Shift_Production * ((latest_entry.production + 1) * 100)) / (main_query.entry_rate * 60) AS Performance,
    main_query.shift,
    (main_query.entry_rate * 100 / (10.5 * 60)) AS Availability,
    CASE
        WHEN main_query.shift_time != 0 THEN FLOOR(main_query.Shift_Production / main_query.shift_time)
        ELSE 0
            ...
            (SELECT COUNT(*)
             ...
             AND (
                 ...
                 (TIME(CONVERT_TZ(NOW(), 'UTC', 'Asia/Colombo')) BETWEEN '00:00:00' AND '07:59:59'
                  AND TIME(time) BETWEEN '20:00:00'
                  AND (SELECT MAX(time) FROM cutting_machine_2 WHERE date = DATE_SUB(DATE(CONVERT_TZ(NOW(), 'UTC', 'Asia/Colombo')), INTERVAL 1 DAY)))
             )
             AND (
